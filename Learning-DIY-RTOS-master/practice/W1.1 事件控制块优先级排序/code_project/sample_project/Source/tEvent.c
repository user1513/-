...
void tEventWait (tEvent * event, tTask * task, void * msg, uint32_t state, uint32_t timeout)
{
    ...
    if (totalTaskCount == 0) {
        ...
        // 有任务等，则遍历寻找一个合适的位置，使得队列按优先级的值从小到大排序
        uint32_t i = 0;
        tNode * currentNode = tListFirst(&event->waitList);
        for (i = 0; i < totalTaskCount; i++) {
            tTask * currentNodeTask = tNodeParent(currentNode, tTask, linkNode);
            if (task->prio < currentNodeTask->prio) {
                tListInsertAfter(&event->waitList, currentNode->preNode, &task->linkNode);
                break;
            }
            currentNode = currentNode->nextNode;
        }

        // 比队列中所有任务优先级都要低？插入到队尾
        if (i >= totalTaskCount) {
            tListAddLast(&event->waitList, &task->linkNode);
        }
    }
