...
static void monitorPinCmd (void) {
    ...
    char * pinCh = strtok(NULL, spaceCh);
    if (pinCh == NULL) {
        showMsg(noEnoughParamMsg);
        return;
    } else {
        ...
        uint8_t pin = atoi(pinCh);
        on_off = strtok(NULL, spaceCh);     // monitor pin pinnum on/off
        if (on_off == NULL) {
            showMsg(noEnoughParamMsg);
            return;
...
static void monitorCmd (void) {
    char *type = strtok(NULL, spaceCh);     // monitor on/off
    if (type == NULL) {
        showMsg(noEnoughParamMsg);
        return;
    }
    ...
    if (strstr(type, "on")) { 
        MonitorOn();
    } else if (strstr(type, "off")) {
        MonitorOff();
    } else if (strstr(type, "pin")) {       // monitor pin ???
        if (MonitorIsOn()) {
            monitorPinCmd();
        } else {
